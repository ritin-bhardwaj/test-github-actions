...
// NAMESPACE OBJECT: ./node_modules/axios/index.js
var node_modules_axios_namespaceObject = {};
__nccwpck_require__.r(node_modules_axios_namespaceObject);
...
async function run() {
  ...
  const IAP_INSTANCE = (0,core.getInput)("IAP_INSTANCE");
  const API_ENDPOINT = (0,core.getInput)("API_ENDPOINT");
  const API_ENDPOINT_BODY = JSON.parse((0,core.getInput)("API_ENDPOINT_BODY"));
  const TIMEOUT = (0,core.getInput)("TIME_INTERVAL");
  const NO_OF_ATTEMPTS = (0,core.getInput)("NO_OF_ATTEMPTS");
  const JOB_STATUS = (0,core.getInput)("JOB_STATUS");
  let count = 0;

  try {
    //check the status of the job and return the output (IAP release <= 2021.1)
    const jobStatus211 = (job_id) => {
      (0,node_modules_axios_namespaceObject.get)(
      ...
      (0,node_modules_axios_namespaceObject.get)(
        ...
        .then((res) => {
          ...
          if (res.data.data.status === "running" && count < NO_OF_ATTEMPTS) {
            ...
            (0,core.setOutput)("results", res.data.data.variables);
          } else if (res.data.data.status === "canceled") {
            (0,core.setFailed)("Job Canceled");
          } else if (res.data.data.status === "error") {
            (0,core.setFailed)(res.data.data.error);
          } else {
            (0,core.setFailed)(
              "Job Timed out based upon user defined TIMEOUT and NO_OF_ATTEMPTS"
            );
    ...
    const startJob = () => {
      (0,node_modules_axios_namespaceObject.get)(`${IAP_INSTANCE}/health/server?token=` + IAP_TOKEN)
        .then((res) => {
          ...
          (0,node_modules_axios_namespaceObject.post)(
